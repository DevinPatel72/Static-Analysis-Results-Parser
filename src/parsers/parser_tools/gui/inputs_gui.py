...
class InputsGUI:
    ...
    def _populate_view(self, inputs=None):
        ...
        self.project_name._add_placeholder()
        self.project_name.set_real_value(parsers.PROJ_NAME)

        # Reset project version
        self.project_version.delete(0, tk.END)
        self.project_version._add_placeholder()
        self.project_version.set_real_value(parsers.PROJ_VERSION)

        # Remove entries from the previous view
        for row_frame, *_ in self.entries:
            row_frame.destroy()
    ...
    def add_entry(self, p_entry=None):
        ...
        scanner_dropdown_placeholder = (
            self._select_scanner(
        ...
        version_textbox = PathInputWithPlaceholder(
            row_frame,
            placeholder="Scanner Version"
        )

        version_textbox.set_real_value(
            extracted_version
        )

        version_textbox.pack(
            side=tk.LEFT,
            padx=5
        )
        ...
        del_btn = tk.Button(
            row_frame,
            text="Delete",
            command=lambda: self.delete_entry(row_frame)
        )
    ...
    def browse_file(self, entry_widget, scanner):
        ...
        if scanner != "Select Scanner...":
            ...
            filter_str = ", ".join(
                f"*{ext}"
                for ext in selected_scanner.valid_ext
            )

            file_filters = (
                f"--file-filter={selected_scanner.sname} files "
                f"({filter_str}) | "
                f"{filter_str.replace(',', '')}"
            )
        ...
        if path:
            ...
            if path in existing_paths:
                messagebox.showwarning(
                    "Duplicate File",
                    "This file path has already been selected."
                )
    ...
    def submit_data(self):
        ...
        for d in results:
            ...
            if key in seen and not self.dupe_detected_submit_again:
                messagebox.showwarning(
                    "Invalid Input",
                    "Duplicate input entries detected. "
                    "If this is intentional, submit again."
                )
        ...
        # Remove and Prepend keys must be carried over
        # from self.results to results
        for result in self.results:
            for r in results:
                if (
                    r[InputDictKeys.PATH.value]
                    == result[InputDictKeys.PATH.value]
                    and
                    r[InputDictKeys.SCANNER.value]
                    == result[InputDictKeys.SCANNER.value]
                ):
                    if InputDictKeys.REMOVE.value in result:
                        r[InputDictKeys.REMOVE.value] = (
                            result[InputDictKeys.REMOVE.value]
                        )

                    if InputDictKeys.PREPEND.value in result:
                        r[InputDictKeys.PREPEND.value] = (
                            result[InputDictKeys.PREPEND.value]
                        )
