...
class JsonInputPreviewGUI:
    ...
    def _load_json_files(self):
        ...
        for filename in sorted(os.listdir(self.json_folder)):
            ...
            self.project_files[filename] = full_path
    ...
    def _on_file_selected(self, event):
        ...
        try:
            ...
            preview_text = self._build_preview(data)

        except Exception as ex:
            preview_text = (f"Failed to load JSON file:\n\n{ex}")
    ...
    def _build_preview(self, data):
        ...
        if not scanner_inputs:
            lines.append("No scanner inputs defined.")
        else:
            for idx, item in enumerate(
                ...
                lines.append(
                    f"   Scanner      : "
                    f"{item.get(InputDictKeys.SCANNER.value, '')}"
                )

                lines.append(
                    f"   File Path    : "
                    f"{item.get(InputDictKeys.PATH.value, '')}"
                )

                lines.append(
                    f"   Remove Path  : "
                    f"{item.get(InputDictKeys.REMOVE.value, '')}"
                )

                lines.append(
                    f"   Prepend Path : "
                    f"{item.get(InputDictKeys.PREPEND.value, '')}"
                )
