...
| `-I`<br>`--extended-input SCANNER FILE REMOVE PREPEND`                             | Add a scanner input with path normalization settings. Accepts scanner name, file path, path prefix to remove, and path prefix to prepend. Can be specified multiple times. Used in addition to a `--file` input if present.      |
...
| `--no-security-preflight-rules`                                                    | Disable the built-in Security Preflight Rules. Overrides the flag value specified in a `--file`  put.                                                                                                                             |
...
| `-j`<br>`--jobs [JOBS]`                                                            | Define the number of processors to complete parsing. By default uses 1 processor.                                                                                                                                                 |
